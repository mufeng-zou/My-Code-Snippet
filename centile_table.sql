with a as (
SELECT *, case when PERCENT_RANK() over (order by score)=0 then 1 else ceiling(PERCENT_RANK() over (order by score)*10) end as perc
...
(
 (SELECT MAX(score) FROM
   (SELECT TOP 50 PERCENT score FROM a where a.perc=b.perc ORDER BY score) AS BottomHalf)
 +
 (SELECT MIN(Score) FROM
   (SELECT TOP 50 PERCENT perc, score FROM a where a.perc=b.perc ORDER BY score DESC) AS TopHalf)
) / 2 as Median_Score
--into #temp
from a b
group by perc